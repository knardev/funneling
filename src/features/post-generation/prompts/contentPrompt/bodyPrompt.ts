...
4. **최종 출력 조건**  
   - 본문은 하나의 텍스트로 연결된 흐름으로 작성되어 JSON의 **optimized_body** 키 안에 포함해야 합니다.  
   - 모든 줄바꿈 및 탭 문자는 \\n 등의 이스케이프 처리되어야 합니다.  
...
본론은 독자가 이해하기 쉽고, 흥미를 유지하며, 실제적인 통찰과 정보를 제공할 수 있어야 합니다.  
본문은 목차를 기반으로 자연스럽게 구성되며, 브랜드의 주제와 서비스 가치를 포함하여 독자에게 매력적인 문장과 구체적인 정보를 전달하도록 최적화합니다.  
주의: 본론은 글의 마무리나 결론을 포함하지 않아야 하며, 결론은 따로 생성됩니다.  
**반드시 최소 4000자 이상의 본문을 작성할 것**
...
1. **제목, 주제, 서론 및 목차 분석**  
   - 제목과 주제가 전달하는 핵심 메시지와 목차의 흐름을 단순화하여 분석합니다.  
   - 브랜드의 서비스 가치와 차별점을 고려하여 독자가 얻을 핵심 인사이트를 도출합니다.  
   - 독자의 주요 궁금증이나 니즈 2-3개를 도출하고 감정적 연결과 신뢰성을 확인합니다.

2. **YES-SET 구성 및 Writing Flow**  
   - 아래 순서대로 글을 풀어나가야 함을 명심합니다:
     - **Problem (문제 제기):** 서비스분석을 참고하여 문제를 환기하고, 독자의 니즈를 짚어줍니다.
     - **Affinity (친근감):** 독자와 공감할 수 있는 멘트로 친근감을 형성합니다.
     - **Solution (해결책):** 서비스분석과 자연스럽게 서비스 가치를 활용하여 해결책을 제안합니다.
     - **Ofeer (제안):** 구체적인 제안을 추가합니다.
     - **Narrosing down (제한):** 내용을 제한하여 명확한 포인트를 전달합니다.
     - **Action (행동):** 독자가 취할 행동을 유도하는 마무리 문장을 작성합니다.
     
3. **본론 작성 가이드**  
   - 모든 문장은 쉽고, 짧고, 구체적이며 읽기 쉽게 작성합니다.  
   - 실제 사례나 경험을 통해 내용을 구체적으로 설명합니다.  
   - 브랜드의 주제와 서비스 가치를 자연스럽게 녹여내어 독자의 관심을 유도합니다.

4. **최종 출력 조건**  
   - 본문은 하나의 텍스트 흐름으로 작성되어 JSON의 **optimized_body** 키 안에 포함해야 합니다.  
   - 모든 줄바꿈 및 탭 문자는 \\n 등으로 이스케이프 처리되어야 합니다.  
   - 반드시 아래 JSON 구조로만 출력할 것:
