    ...
    try {
        ...
        const extractedSections: Section[] = await page.evaluate(() => {
            ...
            subjectBoxes.forEach((box, index) => {
                ...
                // '기타' 유형이 아니고 '인기주제'가 포함되지 않은 섹션만 추가
                // 하지만 순서는 전체 순서를 유지
                if (sectionType !== '기타' && !tabTitle.includes('인기주제')) {
                    result.push({
                        order: index + 1, // 전체 순서 유지
                        title: tabTitle,
                        type: sectionType
                    });
                }
...
// 예시 사용법
(async () => {
    const keyword = '블로그 마케팅'; // 검색할 키워드
    const sections = await scrapeNaverSections(keyword);

    if (sections.length > 0) {
        console.log("=== 검색 결과 섹션 목록 ===");
        sections.forEach(section => {
            console.log(`순서: ${section.order}, 섹션 유형: ${section.type}, 탭명: ${section.title}`);
        });
    } else {
        console.log("검색 결과 섹션을 찾을 수 없습니다.");
    }
})();
