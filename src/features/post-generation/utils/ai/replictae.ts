...
export const REPLICATE_CONFIG = {
    API: {
      ...
      KEY: process.env.REPLICATE_API_KEY as string, // 환경 변수에서 API 키 불러오기
      MODEL: "black-forest-labs/flux-dev", // 사용하려는 모델 이름
  ...
  export async function makeReplicateRequest(
    ...
    try {
      ...
      // 결과 대기
      const output = await waitForResult(prediction);
  
      // Transformer가 제공된 경우 변환
        
      // Transformer가 없으면 기본 출력 반환
      return prediction;
    } catch (error) {
  ...
  async function makeRequest(
    ...
    try {
      ...
      if (
        ...
        throw new Error(responseData.error?.message || "API 요청 실패");
      }
  ...
  async function startPrediction(input: PredictionInput['prompt']): Promise<PredictionResponse> {
    ...
    const headers = {
      Authorization: `Tocken ${REPLICATE_CONFIG.API.KEY}`,
      "Content-Type": "application/json",
    };
  
    const payload = {
      version: REPLICATE_CONFIG.API.MODEL,
      input:{
        prompt:input,
      }
    };
  ...
  async function waitForResult(prediction: PredictionResponse): Promise<string> {
    const headers = {
      Authorization: `Token ${REPLICATE_CONFIG.API.KEY}`,
    };
    ...
    while (true) {
      ...
      try {
        ...
        switch (result.status) {
          case "succeeded":
            if (result.urls.get && result.urls.get.length > 0) {
